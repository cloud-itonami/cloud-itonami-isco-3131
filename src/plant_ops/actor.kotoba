(ns plant-ops.actor
  "PlantOpsActor — the ISCO-08 3131 power production plant operations
  coordination actor as a `langgraph.graph/state-graph` (per ADR-2607011000
  / CLAUDE.md Actors section). One graph run = one operations coordination
  ...
  COORDINATION WORKFLOW only. It does NOT directly control generators,
  turbines, or grid synchronization — those remain exclusively under
  licensed plant operators' human authority. The Governor enforces this
  hard boundary (see `plant-ops.governor`).
  ...
  The unconditional invariant: the PlantOpsAdvisor can never directly
  commit a record or dispatch a robot action the PlantOpsGovernor refuses
  — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [plant-ops.advisor :as advisor]
            [plant-ops.governor :as governor]
            [plant-ops.store :as store]))

(defn build-graph
  "Build a compiled PlantOpsActor graph. `store` implements
  `plant-ops.store/Store`. `advisor` implements `plant-ops.advisor/Advisor`
