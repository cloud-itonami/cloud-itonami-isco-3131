...
(deftest hard-violations-non-propose-effect
  (testing "non-:propose effect triggers hard violation"
    (let [proposal {:op :log-production-reading :effect :commit}
          request {:plant-id :plant-1}
...
(deftest hard-violations-forbidden-ops
  (testing "generator start is forbidden"
    (let [proposal {:op :generator-start :effect :propose}
          request {:plant-id :plant-1}
          s (store/mem-store {:plant-1 {:name "Plant 1" :status :operational :verified? true}})
          verdict (gov/check request nil proposal s)]
      (is (:hard? verdict))))
  (testing "turbine control is forbidden"
    (let [proposal {:op :turbine-control :effect :propose}
          request {:plant-id :plant-1}
          s (store/mem-store {:plant-1 {:name "Plant 1" :status :operational :verified? true}})
          verdict (gov/check request nil proposal s)]
      (is (:hard? verdict))))
  (testing "grid-sync is forbidden"
    (let [proposal {:op :grid-sync :effect :propose}
          request {:plant-id :plant-1}
          s (store/mem-store {:plant-1 {:name "Plant 1" :status :operational :verified? true}})
          verdict (gov/check request nil proposal s)]
      (is (:hard? verdict)))))
...
(deftest escalation-low-confidence
  (testing "low confidence triggers escalation"
    (let [proposal {:op :log-production-reading :effect :propose :confidence 0.5}
          request {:plant-id :plant-1}
          s (store/mem-store {:plant-1 {:name "Plant 1" :status :operational :verified? true}})
          verdict (gov/check request nil proposal s)]
      (is (not (:hard? verdict)))
...
(deftest ok-proposal
  (testing "valid proposal with high confidence is ok"
    (let [proposal {:op :log-production-reading :effect :propose :confidence 0.85}
          request {:plant-id :plant-1}
          s (store/mem-store {:plant-1 {:name "Plant 1" :status :operational :verified? true}})
          verdict (gov/check request nil proposal s)]
      (is (not (:hard? verdict)))
