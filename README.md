# cloud-itonami-isco-3131

Open Occupation Blueprint for **ISCO-08 3131**: Power Production Plant Operators.

This repository designs a forkable OSS business for power plant operations coordination: a document-handling and telemetry-monitoring robot performs routine operational logging, maintenance scheduling, and anomalous-condition flagging under a governor-gated actor, so a plant operator keeps its own operational and maintenance history instead of renting a closed plant-management platform.

## What This Does NOT Do

**CRITICAL SCOPE BOUNDARY:** This actor supports plant-operator **back-office coordination workflow only**.

- ✗ This actor does **NOT** directly control generators, turbines, or grid-synchronization equipment.
- ✗ This actor does **NOT** dispatch turbine start/stop, load adjustment, or grid-sync commands.
- ✗ This actor does **NOT** have authority over generator dispatch, excitation control, or synchronization signals.

Those capabilities remain exclusively under **licensed plant engineers' and operators' human authority** and are outside this system's vocabulary. The Governor enforces this hard boundary (see `plant-ops.governor`).

## Robotics premise

All cloud-itonami verticals are designed on the premise that a **robot performs the physical domain work**. Here a telemetry-monitoring robot performs routine production readings, maintenance scheduling, and anomalous-condition flagging under an actor that proposes coordination actions and an independent **Plant Operations Governor** that gates them. The governor never dispatches a plant operation itself; `:high`/`:safety-critical` actions (such as anomalous-reading escalation, or any proposal touching generator/turbine control) require human sign-off.
...
plant registration + operational parameters + telemetry observation
        |
        v
Plant Operations Advisor -> Plant Operations Governor -> operational record, maintenance scheduling, escalation, or human approval
        |
        v
robot actions (gated) + operation records + escalation records + audit ledger
```

No automated advice can dispatch a plant operation the governor refuses, suppress an operational record, escalate a safety concern without governor approval, or propose generator/turbine control without hard rejection and audit evidence.

## Capability layer

Resolves via [`kotoba-lang/occupation`](https://github.com/kotoba-lang/occupation) (ISCO-08 `3131`). Required capabilities:
...
- `:log-production-reading` — routine output/parameter reading logging (e.g., MW output, temperature, pressure)
- `:schedule-maintenance` — maintenance scheduling proposal (e.g., routine inspection, component replacement schedule)
...
3. **No generator/turbine/grid-sync control** — any proposal whose `:op` is `:generator-start`, `:generator-stop`, `:turbine-control`, or `:grid-sync` is permanently rejected (those are exclusively licensed operator authority).
